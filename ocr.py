...
for puzzle in os.listdir("puzzles"):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 10,
                            param1=50, param2=20, minRadius=9, maxRadius=14)

    def create_color_grid(colors_list, grid_width, grid_height):
        colors_list = sorted(colors_list, key=lambda x: x[1])
        grid = [sorted(colors_list[h * grid_width : h * grid_width + grid_width], key=lambda x: x[0]) for h in range(0, grid_height)]
        return grid
